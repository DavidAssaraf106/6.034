...
def path_length(graph, path):
    ...
    for i, node in enumerate(path):
        if i + 1 < len(path):
            following_node = path[i + 1]
            weight_edge = graph.get_edge(node, following_node).length
            weight_path += int(weight_edge)
    return weight_path
...
def extensions(graph, path):
    ...
    neighboring_nodes = graph.get_neighbors(terminal_node)
    extended_paths = []
    for candidate in neighboring_nodes:
        path_candidate = path.copy()
        path_candidate.append(candidate)
        if not has_loops(path_candidate):
            extended_paths.append(path_candidate)
...
def sort_by_heuristic(graph, goalNode, nodes):
    ...
    for node in nodes:
        ...
        list_heuristics.append(heuristic)
    return [x for _, x in sorted(zip(list_heuristics, nodes))]
...
def basic_dfs(graph, startNode, goalNode):
    ...
    """
    temporary_stack = []  # will behave like a stack containint lists (=paths), with the head pointer at the end of the list
    temporary_stack.append([startNode])
    while len(temporary_stack) > 0:
        path_to_be_extended = temporary_stack.pop()
        current_terminal_node = path_to_be_extended[-1]
        if current_terminal_node == goalNode:
            return path_to_be_extended
        extensions_path = extensions(graph, path_to_be_extended)
        if extensions_path is not None:
            for path_extended in extensions_path[::-1]:
                temporary_stack.append(path_extended)
    return None
...
def basic_bfs(graph, startNode, goalNode):
    ...
    while len(temporary_stack) > 0:
        ...
        extensions_path = extensions(graph, path_to_be_extended)
        if extensions_path is not None:
            for path_extended in extensions_path:
                temporary_stack.insert(0, path_extended)
    return None
...
def alphabetical_sorting(graph, goalNode, paths):
    alphabetical_sort = sorted(paths)
...
def sort_by_length(graph, goalNode, paths):
    ...
    return [x for _, x in sorted(zip(list_lengths, paths))]


def sort_by_length_heuristic(graph, goalNode, paths):
    list_lengths_heuristics = []
    for path in paths:
        length = path_length(graph, path)
        heuristic = graph.get_heuristic_value(path[-1], goalNode)
        list_lengths_heuristics.append(length+heuristic)
    return [x for _, x in sorted(zip(list_lengths_heuristics, paths))]


def sort_by_heuristic_terminal_node(graph, goalNode, paths):
    list_heuristics = []
    for path in paths:
        heuristic = graph.get_heuristic_value(path[-1], goalNode)
        list_heuristics.append(heuristic)
    return [x for _, x in sorted(zip(list_heuristics, paths))]


generic_dfs = [alphabetical_sorting, True, do_nothing_fn, False]

generic_bfs = [alphabetical_sorting, False, do_nothing_fn, False]

generic_hill_climbing = [sort_by_heuristic_terminal_node, True, do_nothing_fn, False]

generic_best_first = [do_nothing_fn, True, sort_by_heuristic_terminal_node, False]

generic_branch_and_bound = [do_nothing_fn, False, sort_by_length, False]

generic_branch_and_bound_with_heuristic = [do_nothing_fn, False, sort_by_length_heuristic, False]

generic_branch_and_bound_with_extended_set = [do_nothing_fn, False, sort_by_length, True]

generic_a_star = [do_nothing_fn, False, sort_by_length_heuristic, True]
...
def is_admissible(graph, goalNode):
    ...
    for node in graph.nodes:
        length_to_goal = path_length(graph, generic_search(*generic_branch_and_bound_with_extended_set)(graph, node, goalNode))
        heuristic = graph.get_heuristic_value(node, goalNode)
        if heuristic > length_to_goal:
            return False
    return True
...
def is_consistent(graph, goalNode):
    ...
    for edge in graph.get_edges():
        if np.abs(graph.get_heuristic_value(edge.startNode, goalNode) - graph.get_heuristic_value(edge.endNode, goalNode))>edge.length:
            return False
    return True
...
WHAT_I_FOUND_INTERESTING = 'Generic Search'
WHAT_I_FOUND_BORING = ''
SUGGESTIONS = 'Visualize computationnaly the improvements in performance for BB, BB with Extended Set and A*'
